...
class Display:
    ...
    def display(self):
        ...
        if mode == "ask":
            ...
            self.z = buttons.Button(self.wordle, 255, 605, self.wordle.stats.state['z'], "Z", 40, 40)
            self.x = buttons.Button(self.wordle, 305, 605, self.wordle.stats.state['x'], "X", 40, 40)
            self.c = buttons.Button(self.wordle, 355, 605, self.wordle.stats.state['c'], "C", 40, 40)
            self.v = buttons.Button(self.wordle, 405, 605, self.wordle.stats.state['v'], "V", 40, 40)
            self.b = buttons.Button(self.wordle, 455, 605, self.wordle.stats.state['b'], "B", 40, 40)
            self.n = buttons.Button(self.wordle, 505, 605, self.wordle.stats.state['n'], "N", 40, 40)
            self.m = buttons.Button(self.wordle, 555, 605, self.wordle.stats.state['m'], "M", 40, 40)
            self.back = buttons.Button(self.wordle, 705, 555, (255, 255, 0), "<-", 40, 40)
            self.submit = buttons.Button(self.wordle, 705, 605, (255, 255, 0), "ok", 40, 40)
            if self.wordle.stats.cc:
                self.cc = buttons.Button(self.wordle, 750, 550, (255, 255, 0), "COPY EMOJI", 250, 100)
                self.cc.draw_button()
                self.si = buttons.Button(self.wordle, 0, 550, (255, 255, 0), "SAVE IMAGE", 250, 100)
                self.si.draw_button()
            if (not self.wordle.stats.win) and (self.wordle.stats.round != 1) and (self.wordle.stats.round != 6 or not self.wordle.stats.win):
                self.give = buttons.Button(self.wordle, 605, 605, (0, 139, 0), "ANS", 90, 40)
                self.give.draw_button()
            else:
                self.give = buttons.Button(self.wordle, 605, 605, (125, 125, 125), "", 90, 40)
                self.give.draw_button()
            self.a.draw_button()
            self.b.draw_button()
            self.c.draw_button()
